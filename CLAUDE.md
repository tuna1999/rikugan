...
- **Skills**: Markdown + YAML frontmatter trong `rikugan/skills/builtins/<slug>/SKILL.md`. User tự thêm vào `~/.idapro/rikugan/skills/`. 12 skill built-in: `malware-analysis`, `linux-malware`, `deobfuscation`, `ctf`, `modify`, `smart-patch-ida`, `vuln-audit`, `ida-scripting`, `ida-docs`, `ida-pro-mcp`, `driver-analysis`, `generic-re`.
