...
Branch off `dev` với prefix `feat/`, `fix/`, `refactor/`, `chore/`. Commit format: `type(scope): description`. **Không push trực tiếp vào `main`** (bị branch protection chặn). Release flow: merge `dev → main` → bump `version` trong `ida-plugin.json` → tag `v*`.
...
- **Skills**: Markdown + YAML frontmatter trong `rikugan/skills/builtins/<slug>/SKILL.md`. User tự thêm vào `~/.idapro/rikugan/skills/`. 12 skill built-in (malware-analysis, deobfuscation, ctf, modify, vuln-audit, ida-scripting, ...)
